<script lang="ts">
  ...
  async function salvarProprietario() {
    ...
    if (response.ok) {
      message.set('Proprietário salvo com sucesso!');
      nome = '';
      telefone = '';
      email = '';
      endereco = '';
      setTimeout(() => {
        message.set('');
        navigateTo('/proprietarios');
      }, 2000);
    } else {
      message.set('Erro ao salvar o proprietário.');
    }
  }
</script>
...
<div class="container">
  <nav class="nav">
    ...
    <button on:click={() => navigateTo('/proprietarios')}>Voltar</button>
  </nav>
  ...
  <div class="form-group">
    <button on:click={salvarProprietario}>Salvar Proprietário</button>
  </div>
  {#if $message}
