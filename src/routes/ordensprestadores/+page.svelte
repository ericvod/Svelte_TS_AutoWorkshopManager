...
<div class="container">
  ...
  <button class="button" on:click={navigateToCadastroOrdemPrestador}
    >Cadastrar Nova Ordem de Prestador</button
  >
  <div>
    {#each $ordensPrestadores as ordem}
      <div class="ordemprestador">
        <p>
          <strong>Ordem de Serviço:</strong> OS #{ordem.os_id} - Veículo: {ordem
            .ordemDeServico.placa}
        </p>
        <p><strong>Prestador:</strong> {ordem.prestador.nome}</p>
        <p>
          <strong>Data de Início:</strong>
          {new Date(ordem.data_inicio).toLocaleDateString()}
        </p>
        {#if ordem.data_fim}
          <p>
            <strong>Data de Fim:</strong>
            {new Date(ordem.data_fim).toLocaleDateString()}
          </p>
        {/if}
