<script lang="ts">
    ...
    async function salvarItemOrdemServico() {
      ...
      if (response.ok) {
        message.set('Item da Ordem de Serviço salvo com sucesso!');
        ordem_servico_id = '';
        servico_id = '';
        preco_total = '';
        setTimeout(() => {
          message.set('');
          navigateTo('/itensordemservico');
        }, 2000);
      } else {
        message.set('Erro ao salvar o item da ordem de serviço.');
      }
    }
  </script>
  ...
  <div class="container">
    <nav class="nav">
      ...
      <button on:click={() => navigateTo('/itensordemservico')}>Voltar</button>
    </nav>
    <h2>Cadastro de Item da Ordem de Serviço</h2>
    ...
    <div class="form-group">
      ...
      <input type="number" id="servico_id" bind:value={servico_id} placeholder="Digite o ID do serviço" />
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      <button on:click={salvarItemOrdemServico}>Salvar Item</button>
    </div>
    {#if $message}
